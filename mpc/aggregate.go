...
func (netObj *Network) AggregateRefreshShareMatPtr(share [][]*ring.Poly, outLevel int) (shareOut [][]*ring.Poly) {
	n := len(share)
	contextQ := netObj.dckksContext.RingQ
	shareOut = make([][]*ring.Poly, n)
	for i := range shareOut {
		shareOut[i] = make([]*ring.Poly, len(share))
	}
		...
		if pid == netObj.hubPid {
			...
			for p := 1; p < netObj.NumParties; p++ {
				...
				for i := range share {
					for j := range share[i] {
						...
						contextQ.AddLvl(len(other.Coeffs)-1, other, shareOut[i][j], shareOut[i][j])
					}
				}
			}
			...
			for p := 1; p < netObj.NumParties; p++ {
				if p != pid {
					for i := range share {
						for j := range share[i] {
							netObj.SendPoly(shareOut[i][j], p)
						}
					}
				}
			}
			...
			for i := range share {
				for j := range share[i] {
					netObj.SendPoly(share[i][j], netObj.hubPid)
				}
			}
			...
			for i := range share {
				for j := range share[i] {
					shareOut[i][j] = netObj.ReceivePoly(netObj.hubPid)
				}
			}
		}
	}
...
func (netObj *Network) AggregateRefreshShareVec(share []*ring.Poly, outLevel int) (shareOut []*ring.Poly) {
	shareMat := make([][]*ring.Poly, 1)
	shareMat[0] = share
	return netObj.AggregateRefreshShareMatPtr(shareMat, outLevel)[0]
}
