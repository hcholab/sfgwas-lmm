...
func InitParallelMPCEnv(netObjs []*Network, rtype mpc_core.RElem, dataBits, fracBits int) []*MPC {
	...
	for i := 1; i < len(netObjs); i++ {
		lagrangeCopy := make(map[TypedKey]mpc_core.RMat)
		for k, v := range lagrangeCache {
			lagrangeCopy[k] = v.Copy()
		}
		mpcEnvParallel[i] = &MPC{
			Network:         netObjs[i],
			dataBits:        dataBits,
			fracBits:        fracBits,
			divSqrtMaxLen:   100000,
			orLagrangeCache: make(map[TypedKey]mpc_core.RVec),
			lagrangeCache:   lagrangeCopy,
...
func initMPCEnv(netObj *Network, rtype mpc_core.RElem, dataBits, fracBits int) *MPC {
	gob.Register(mpc_core.LElem256{})
	gob.Register(mpc_core.LElem128{0, 0})
...
func (mpcObj *MPC) GetPascalMatrix(rtype mpc_core.RElem, pow int) mpc_core.RMat {
	...
	if found {
		// log.LLvl1(time.Now(), "status of cache: ", pascalCache)
		return res
	}
...
func (mpcObjs ParallelMPC) GetHubPid() int {
	return mpcObjs[0].Network.hubPid
}
...
func (mpcObjs ParallelMPC) GetNParty() int {
	return mpcObjs[0].Network.NumParties
}
...
func (mpcObj *MPC) RevealSymMat(a mpc_core.RMat) mpc_core.RMat {
	...
	// 	ch := make(chan mpc_core.RMat, mpcObj.Network.NumParties-1)

	// 	start := time.Now()
	// 	for p := 1; p < mpcObj.Network.NumParties; p++ {
	// 		if p != pid {
	// 			go func(pid int) {
	// 				r := mpcObj.Network.ReceiveRMat(rtype, nr, nc, pid)
	// 				ch <- r
	// 				// ar.Add(r)
	// 			}(p)
	// 		}
	// 	}
	// 	receive := time.Since(start)

	// 	totalDuration, _ := time.ParseDuration("1ns")
	// 	for p := 1; p < mpcObj.Network.NumParties-1; p++ {
	// 		r := <-ch
	// 		startindiv := time.Now()
	// 		ar.Add(r)
	// 		dur := time.Since(startindiv)
	// 		totalDuration += dur
	// 	}
	// 	start = time.Now()
	...
	// 			go func(pid int) {
	// 				defer wg.Done()
	// 				mpcObj.Network.SendRData(ar, pid)
	// 			}(p)
	// 		}
	// 	}
	// 	wg.Wait()
	// 	log.LLvl1(time.Now().Format(time.StampMilli), "Network recieve:", receive, ", Adds: ", totalDuration, "Network Send: ", time.Since(start))
	...
	if pid == mpcObj.GetHubPid() {
		...
		for p := 1; p < mpcObj.Network.NumParties; p++ {
			if p != pid {
				mpcObj.Network.SendRData(ar, p)
			}
		}
		return ar
	}

	mpcObj.Network.SendRData(a, mpcObj.GetHubPid())
	return mpcObj.Network.ReceiveRMat(rtype, nr, nc, mpcObj.GetHubPid())
}
...
func (mpcObj *MPC) RevealSymMatParallel(a []mpc_core.RMat) []mpc_core.RMat {
	...
	if pid == mpcObj.GetHubPid() {
		...
		for p := 1; p < mpcObj.Network.NumParties; p++ {
			if p != pid {
				for i := range out {
					mpcObj.Network.SendRData(out[i], p)
				}
			}
		}
	...
	// Send
	for i := range out {
		mpcObj.Network.SendRData(a[i], mpcObj.GetHubPid())
	}
	...
	for i := range out {
		nr, nc := a[i].Dims()
		rtype := a[i].Type()
		out[i] = mpcObj.Network.ReceiveRMat(rtype, nr, nc, mpcObj.GetHubPid())
	}
...
func (mpcObj *MPC) EvaluatePoly(a mpc_core.RVec, coeff mpc_core.RMat) mpc_core.RMat {
	...
	out := mpc_core.RMultMat(coeff, apow)
	return out
}
...
func (mpcObj *MPC) TableLookup(a mpc_core.RVec, tableId int) mpc_core.RMat {
	...
	if !found {
		panic(fmt.Sprintf("Requested table for TableLookup is invalid:", key))
	}
...
func (mpcObj *MPC) NormalizerEvenExp(x mpc_core.RVec, k int) (mpc_core.RVec, mpc_core.RVec) {
	...
	if rtypeInput.TypeID() == mpc_core.LElem256UniqueID {
		...
		panic("Unsupported RElem type for NormalizerEvenExp")
	}
...
// 	log.LLvl1(time.Now().Format(time.StampMilli), "share random bits, ", time.Since(start))
// 	start = time.Now()
// 	e := a.Copy()
// 	e.Add(r)
// 	r = nil
...
// 	e = nil
// 	log.LLvl1(time.Now().Format(time.StampMilli), "revealk e, ", time.Since(start))
// 	start = time.Now()
...
// 		c = cNew
// 	}
// 	log.LLvl1(time.Now().Format(time.StampMilli), "LessThanBitsPublic, ", time.Since(start))
// 	start = time.Now()
...
// 	}
// 	c = nil
// 	log.LLvl1(time.Now().Format(time.StampMilli), "ep, ", time.Since(start))
// 	start = time.Now()
// 	E := mpcObj.PrefixOr(ep)
// 	log.LLvl1(time.Now().Format(time.StampMilli), "prefixOr, ", time.Since(start))
// 	start = time.Now()
...
// 	tpNeg = nil
// 	log.LLvl1(time.Now().Format(time.StampMilli), "TNeg, ", time.Since(start))
// 	start = time.Now()
// 	halfLen := int(k / 2)
...
// 	}
// 	TNeg = nil
// 	log.LLvl1(time.Now().Format(time.StampMilli), "odd even, ", time.Since(start))
// 	start = time.Now()
...
// 	log.LLvl1(time.Now().Format(time.StampMilli), "add, ", time.Since(start))
// 	start = time.Now()
// 	bMat := mpcObj.TableLookupWithShareConversion(rtype, chosenBitSum, 1)
// 	log.LLvl1(time.Now().Format(time.StampMilli), "TableLookupWithShareConversion, ", time.Since(start))
// 	start = time.Now()
...
func (mpcObj *MPC) BinaryPrefixOr(a mpc_core.RMat, numBits int) mpc_core.RMat {
	...
	for round := 1; round <= numRounds; round++ {
		if round <= 6 { // Shift stays within each uint64
			...
			sor, som := mpcObj.BeaverPartitionMatParallel([]mpc_core.RMat{move, out})
			sr, sm := sor[0], som[0]
			or, om := sor[1], som[1]
			...
			rConcat, mConcat := mpcObj.BeaverPartitionMatParallel([]mpc_core.RMat{propIn, propOut})
			dPSr, dPSm := rConcat[0], mConcat[0]
			dPr, dPm := rConcat[1], mConcat[1]
...
func (mpcObj *MPC) SuffixCarryAux(dProp mpc_core.RMat, dGen mpc_core.RMat, numBits int) mpc_core.RMat {
	...
	for round := 1; round <= numRounds; round++ {
		if round <= 6 { // Shift stays within each uint64
			...
			// Multiply with previous carry bits
			rConcat, mConcat := mpcObj.BeaverPartitionMatParallel([]mpc_core.RMat{dPropMove, dGenMove, dProp})
			dPSr, dPSm := rConcat[0], mConcat[0]
			dGSr, dGSm := rConcat[1], mConcat[1]
			dPr, dPm := rConcat[2], mConcat[2]
			...
			rConcat, mConcat := mpcObj.BeaverPartitionMatParallel([]mpc_core.RMat{propIn, genIn, propOut})
			dPSr, dPSm := rConcat[0], mConcat[0]
			dGSr, dGSm := rConcat[1], mConcat[1]
			dPr, dPm := rConcat[2], mConcat[2]
...
func (mpcObj *MPC) CarryOutAux(dProp mpc_core.RMat, dGen mpc_core.RMat, numBits int) mpc_core.RVec {
	...
	if numBits == 1 {
		...
		rConcat, mConcat := mpcObj.BeaverPartitionMatParallel([]mpc_core.RMat{dPropShift, dProp, dGen})
		dPSr, dPSm := rConcat[0], mConcat[0]
		dPr, dPm := rConcat[1], mConcat[1]
		dGr, dGm := rConcat[2], mConcat[2]
...
func (mpcObj *MPC) IsPositive(x mpc_core.RVec) mpc_core.RVec {
	...
	if rtypeInput.TypeID() == mpc_core.LElem256UniqueID {
		...
		panic("Unsupported RElem type for IsPositive")
	}
...
// func (mpcObj *MPC) IsPositive(a mpc_core.RVec) mpc_core.RVec {
// 	pid := mpcObj.GetPid()
...
func (mpcObj *MPC) SqrtAndSqrtInverse(a mpc_core.RVec) (b, bInv mpc_core.RVec) {
	if len(a) > mpcObj.divSqrtMaxLen {
		...
		for remainder, shift := n, 0; remainder > 0; {
			...
			out, outInv := mpcObj.SqrtAndSqrtInverse(a[start:end])
...
func (mpcObj *MPC) Divide(a, b mpc_core.RVec) mpc_core.RVec {
	if len(a) > mpcObj.divSqrtMaxLen {
		...
		for remainder, shift := n, 0; remainder > 0; {
			...
			log.LLvl1(time.Now(), fmt.Sprintf("MPC division on large vector (%d-%d / %d)", start, end, n))

			copy(c[start:end], mpcObj.Divide(a[start:end], b[start:end]))
	...
	for it := 0; it < numIter; it++ {
		...
		xyr, xym := mpcObj.BeaverPartitionMat(mpc_core.RMat{x, y})
		xr, xm := xyr[0], xym[0]
		yr, ym := xyr[1], xym[1]
...
func (mpcObj *MPC) Householder(x mpc_core.RVec) mpc_core.RVec {
	...
	xnorm, _ := mpcObj.SqrtAndSqrtInverse(mpc_core.RVec{xdot})
	x1sign := mpcObj.IsPositive(mpc_core.RVec{x[0]})
	...
	_, vnormInv := mpcObj.SqrtAndSqrtInverse(mpc_core.RVec{vdot})
...
func (mpcObjs ParallelMPC) SSMultElemVec(a, b mpc_core.RVec) mpc_core.RVec {
	return mpcObjs.runParallel(mpc_core.RMat{a, b}, nil,
		func(mpc *MPC, mat mpc_core.RMat, aux mpc_core.RElem) mpc_core.RVec {
...
func (mpcObjs ParallelMPC) LessThan(a, b mpc_core.RVec) mpc_core.RVec {
	return mpcObjs.runParallel(mpc_core.RMat{a, b}, nil,
		func(mpc *MPC, mat mpc_core.RMat, aux mpc_core.RElem) mpc_core.RVec {
			return mpc.LessThan(mat[0], mat[1])
		})
}
...
func (mpcObjs ParallelMPC) SqrtInv(a mpc_core.RVec) mpc_core.RVec {
	return mpcObjs.runParallel(mpc_core.RMat{a}, nil,
		func(mpc *MPC, mat mpc_core.RMat, aux mpc_core.RElem) mpc_core.RVec {
			_, res := mpc.SqrtAndSqrtInverse(mat[0])
			return res
		})
}
...
func (mpcObjs ParallelMPC) IsPositive(a mpc_core.RVec) mpc_core.RVec {
	return mpcObjs.runParallel(mpc_core.RMat{a}, nil,
		func(mpc *MPC, mat mpc_core.RMat, aux mpc_core.RElem) mpc_core.RVec {
			return mpc.IsPositive(mat[0])
		})
}

func (mpcObjs ParallelMPC) runParallel(a mpc_core.RMat, aux mpc_core.RElem, fn MpcRoutine) mpc_core.RVec {
	...
	for i := 0; i < numThreads; i++ {
		...
		if endIndex > startIndex {
			wg.Add(1)
			go func(threadID, startIndex, endIndex int) {
				defer wg.Done()
				...
				log.LLvl1(fmt.Sprintf("runParallel: processed %d-%d / %d", startIndex, endIndex, n))
...
func (mpcObjs ParallelMPC) MatrixInverseSVD(A []mpc_core.RMat) (AInv []mpc_core.RMat) {
	...
	numThreads := len(mpcObjs)
	var wg sync.WaitGroup
	Ainv := make([]mpc_core.RMat, len(A))
	rounds := ((numMat - 1) / numThreads) + 1
	for i := 0; i < rounds; i++ {
		for thread := 0; thread < numThreads; thread++ {
			threadID := thread + i*numThreads
			if threadID < numMat {
				wg.Add(1)
				log.LLvl1(fmt.Sprintf("Matrix inverse runParallel: going to process %d", threadID))
				go func(thread, threadID int) {
					defer wg.Done()
					Ainv[threadID] = mpcObjs[thread].MatrixInverseSVD(A[threadID])
					log.LLvl1(fmt.Sprintf("Matrix inverse runParallel: processed %d", threadID))
				}(thread, threadID)
			}
		}
		wg.Wait()
...
func (mpcObj *MPC) MatrixInverseSVD(A mpc_core.RMat) (AInv mpc_core.RMat) {
	...
	_, SInv := mpcObj.SqrtAndSqrtInverse(S2)
...
func (mpcObj *MPC) MatrixInverseSymPos(ASymPosDef mpc_core.RMat) (AInv, AInvSqrt mpc_core.RMat) {
	...
	_, LInvSqrt := mpcObj.SqrtAndSqrtInverse(L)
