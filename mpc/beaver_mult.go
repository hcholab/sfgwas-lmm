...
func (mpcObj *MPC) BeaverPartitionMatParallel(a []mpc_core.RMat) ([]mpc_core.RMat, []mpc_core.RMat) {
	...
	if pid == 0 {
		ar := make([]mpc_core.RMat, len(a))
		am := make([]mpc_core.RMat, len(a))
		for i := range a {
			nrows, ncols := a[i].Dims()
			rtype := a[i].Type()
			ar[i] = mpc_core.InitRMat(rtype.Zero(), nrows, ncols)
			am[i] = mpc_core.InitRMat(rtype.Zero(), nrows, ncols)
		}
	...
	// Retrieve random shares from pid = 0
	mask := make([]mpc_core.RMat, len(a))

	mpcObj.Network.Rand.SwitchPRG(0)
	for i := range a {
		nrows, ncols := a[i].Dims()
		rtype := a[i].Type()
		mask[i] = mpcObj.Network.Rand.RandMat(rtype, nrows, ncols)
	}
	mpcObj.Network.Rand.RestorePRG()
	...
	ar = mpcObj.RevealSymMatParallel(ar)
	return ar, mask
}
