...
func (mpcObj *MPC) SSMultMatParallel(a, b []mpc_core.RMat) []mpc_core.RMat {
	n := len(a)
	ar, am := mpcObj.BeaverPartitionMatParallel(a)
	br, bm := mpcObj.BeaverPartitionMatParallel(b)

	ab := make([]mpc_core.RMat, n)
	for i := range ab {
		ab[i] = mpcObj.BeaverMultMat(ar[i], am[i], br[i], bm[i])
		ab[i] = mpcObj.BeaverReconstructMat(ab[i])
	}
	return ab
}
...
func (mpcObj *MPC) SSMultElemVec(a, b mpc_core.RVec) mpc_core.RVec {
	r, m := mpcObj.BeaverPartitionMat(mpc_core.RMat{a, b})
	x := mpcObj.BeaverMultElemVec(r[0], m[0], r[1], m[1])
...
func (mpcObj *MPC) CMatToSSAll(cryptoParams *crypto.CryptoParams, cm crypto.CipherMatrix, numCtxRow, numCtxCol, nElemRow int) (ss []mpc_core.RMat) {
	ss = make([]mpc_core.RMat, mpcObj.GetNParty()-1)
	for p := range ss {
		ss[p] = mpcObj.CMatToSS(cryptoParams, mpcObj.GetRType(), cm, p+1, numCtxRow, numCtxCol, nElemRow)
	}
	return
}
...
func (mpcObjs ParallelMPC) CMatToSSAll(cryptoParams *crypto.CryptoParams, cm crypto.CipherMatrix, numCtxRow, numCtxCol, nElemRow int) (ss []mpc_core.RMat) {
	ss = make([]mpc_core.RMat, mpcObjs.GetNParty()-1)
	numParties := mpcObjs.GetNParty() - 1
	numThreads := len(mpcObjs)
	var wg sync.WaitGroup
	rounds := ((numParties - 1) / numThreads) + 1
	for i := 0; i < rounds; i++ {
		for thread := 0; thread < numThreads; thread++ {
			threadID := thread + i*numThreads
			if threadID < numParties {
				wg.Add(1)
				go func(cps *crypto.CryptoParams, thread, threadID int) {
					defer wg.Done()
					mpcObj := mpcObjs[thread]
					ss[threadID] = mpcObj.CMatToSS(cps, mpcObj.GetRType(), cm, threadID+1, numCtxRow, numCtxCol, nElemRow)
				}(cryptoParams.CopyNoRotKeys(), thread, threadID)
			}
		}
		wg.Wait()
	}
...
func (mpcObj *MPC) CMatToSS(cryptoParams *crypto.CryptoParams, rtype mpc_core.RElem, cm crypto.CipherMatrix, sourcePid, numCtxRow, numCtxCol, nElemRow int) (rm mpc_core.RMat) {
	...
	agg := mpcObj.Network.AggregateRefreshShareMatPtr(shareDecrypt, levelStart)
...
func (mpcObj *MPC) CVecToSSConcat(cryptoParams *crypto.CryptoParams, rtype mpc_core.RElem, cv crypto.CipherVector, numCtx, nElem int) (rm []mpc_core.RVec) {
	cm := mpcObj.Network.BroadcastCVecConcat(cryptoParams, cv, numCtx)
	return mpcObj.CMatToSS(cryptoParams, rtype, cm, -1, len(cm), numCtx, nElem)
}
