...
func InitCommunication(servers map[string]Server, pid, nparties, numThreads int) []*Network {
	...
	for thread := range network {
		...
		go func(thread int) {
			...
			network[thread] = initNetworkForThread(servers, pid, nparties, thread)
...
func initNetworkForThread(servers map[string]Server, pid int, nparties, thread int) *Network {
	...
	for other := 0; other < nparties; other++ {
		...
		if other < pid { // Act as a client
			...
			ip := servers[pidString(pid)].IpAddr
...
func (netObj *Network) SetMHEParams(params *ckks.Parameters) {
	...
	netObj.Rand.RandRead(crpKey)
	netObj.Rand.RestorePRG()

	prng, _ := utils.NewKeyedPRNG(crpKey)
	crpGen := ring.NewUniformSampler(prng, dckksContext.RingQP)

	p := crpGen.ReadNew()
	log.LLvl1(time.Now(), "Debugging output: crpGen initial state", p.Coeffs[0][0], p.Coeffs[1][1], p.Coeffs[2][2])
