...
input_dir = sys.argv[1] # input_dir + "/group_x/geno_qc.bin"/"count_qc.txt"/"cov_qc.txt"/"pheno_qc.txt"/"pos_qc.txt" should exist
nparties = int(sys.argv[2])
counts = [[int(line) for line in open(os.path.join(input_dir, "group_"+str(i)+"/count_qc_group_"+str(i)+".txt"))] for i in range(nparties)]
...
# find chrids
chr_ids = [line.split()[0] for line in open(os.path.join(input_dir, "group_0/pos_qc_group_0.txt"))]
ind = 0
prev = None
colchrinds = []
for ch in chr_ids:
...
for ch in range(numchr):
    ncolblock += int(math.ceil(float(colchrinds[ch+1]-colchrinds[ch])/float(ncols_per_block)))
...
for p in range(nparties):
    for k in range(nfolds):
        arr = [os.path.join(output_dir, "party"+str(p+1), "fold" + str(k+1) + "." + str(j) + ".bin") for j in range(ncolblock)]
...
for p in range(nparties):
    if p == 0:
        write_block_inds(os.path.join(output_dir, "party"+str(p), "blockSizes.txt"), colblockinds)
        f = open(os.path.join(output_dir, "party"+str(p), "blockToChrom.txt"), 'wt')
        f.write("\n".join(block2chr)+"\n")
        f.close()

    write_block_inds(os.path.join(output_dir, "party"+str(p+1), "blockSizes.txt"), colblockinds)
...
for p in range(nparties):
    if p ==0:
        src = os.path.join(input_dir, "group_"+str(p)+"/pos_qc_group_"+str(p)+".txt")
        dst = os.path.join(output_dir, "party"+str(p),"pos.txt")
        shutil.copyfile(src, dst)
    src = os.path.join(input_dir, "group_"+str(p)+"/pos_qc_group_"+str(p)+".txt")
    dst = os.path.join(output_dir, "party"+str(p+1),"pos.txt")
    shutil.copyfile(src, dst)

for p in range(nparties):
    src = os.path.join(input_dir, "group_"+str(p)+"/ukb_qc_pheno_group_"+str(p)+".txt")
    dst = os.path.join(output_dir, "party"+str(p+1),"pheno.txt")
    shutil.copyfile(src, dst)

for p in range(nparties):
    src = os.path.join(input_dir, "group_"+str(p)+"/ukb_qc_cov_group_"+str(p)+".txt")
    dst = os.path.join(output_dir, "party"+str(p+1),"cov.txt")
    shutil.copyfile(src, dst)

infiles = [open(os.path.join(input_dir, "group_"+str(i)+"/ukb_qc_group_bin_"+str(i)+".bin"), 'rb') for i in range(nparties)]
...
for p in range(nparties):
    ...
    for k in range(nfolds):
        for b in range(ncolblock):
            ...
            arr[int(foldpartyinds[p][k]):int(foldpartyinds[p][k+1]),colblockinds[b]:colblockinds[b+1]].tofile(f)
            f.close()
